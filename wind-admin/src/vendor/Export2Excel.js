...
function hex2ab(value) {
  const typedArray = new Uint8Array(value.match(/[\da-f]{2}/gi).map(function(h) {
    return parseInt(h, 16)
  }))
  let buffer = typedArray.buffer
  const bufLen = buffer.byteLength
  if (Math.floor(bufLen % 20) !== 0) {
    const fillBuffer = new Uint8Array(Math.ceil(bufLen / 20) * 20)
...
export function export_byte_to_excel(byte, defaultTitle) {
  const title = defaultTitle || 'excel-list'
  FileSaver.saveAs(new Blob([hex2ab(byte)], { type: 'application/octet-stream' }), title + '.xlsx')
}
